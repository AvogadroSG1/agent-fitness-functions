...
func rejectUnknownFitnessFunctionSettingsKeys(content []byte) error {
	...
	if err := json.Unmarshal(content, &wrapper); err != nil {
		return fmt.Errorf("parsing fitness function settings: %w", err)
	}
	for name := range wrapper.Settings {
		if !allowedFitnessFunctionSettingsKeys[name] {
			return fmt.Errorf("unsupported fitness function settings %q", name)
		}
	}
	return nil
}
...
func validateFitnessFunctionSettings(config *Config) error {
	if config.FitnessFunctionSettings != nil {
		...
		if settings.TemporalPurity != nil {
			...
			default:
				return fmt.Errorf("unsupported temporal-purity csharp-policy %q", settings.TemporalPurity.CSharpPolicy)
			}
		}
	}
	...
	if config.Enabled("layer-sovereignty") {
		...
		if len(layers) == 0 {
			return errors.New("layer-sovereignty is enabled but fitness-function-settings.layer-sovereignty.layers is empty")
		}
	}
	return nil
}
...
func compileLayerRule(rule LayerRule) (LayerRule, error) {
	if rule.Name == "" {
		return LayerRule{}, errors.New("layer-sovereignty layer is missing a name")
	}
	if len(rule.Paths) == 0 {
		return LayerRule{}, fmt.Errorf("layer-sovereignty layer %q has no paths", rule.Name)
	}
	if len(rule.ForbiddenPatterns) == 0 {
		return LayerRule{}, fmt.Errorf("layer-sovereignty layer %q has no forbidden-patterns", rule.Name)
	}
	...
	for _, glob := range rule.Paths {
		...
		if err != nil {
			return LayerRule{}, fmt.Errorf("layer-sovereignty layer %q has invalid path glob %q: %w", rule.Name, glob, err)
		}
	...
	for _, source := range rule.ForbiddenPatterns {
		...
		if err != nil {
			return LayerRule{}, fmt.Errorf("layer-sovereignty layer %q has invalid forbidden-pattern %q: %w", rule.Name, source, err)
		}
