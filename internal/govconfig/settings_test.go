...
func TestParseConfigContentParsesLayerSovereigntySettings(t *testing.T) {
	...
	config, err := Parse([]byte(settingsConfig))
	if err != nil {
...
func TestParseConfigContentRejectsUnknownSettingsKey(t *testing.T) {
	...
	if err == nil {
		t.Fatal("Parse() error = nil, want unsupported settings key error")
	}
}
...
func TestParseConfigContentRejectsInvalidForbiddenPatternRegex(t *testing.T) {
	...
	if err == nil {
		t.Fatal("Parse() error = nil, want invalid forbidden-pattern regex error")
	}
}
...
func TestParseConfigContentRejectsLayerSovereigntyEnabledWithoutLayers(t *testing.T) {
	...
	for name, content := range map[string]string{
		...
		if _, err := Parse([]byte(content)); err == nil {
			t.Errorf("Parse() error = nil for %s, want layer-sovereignty misconfiguration error", name)
		}
	}
}
...
func TestParseConfigContentRejectsIncompleteLayer(t *testing.T) {
	...
	for name, layer := range map[string]string{
		...
		if _, err := Parse([]byte(content)); err == nil {
			t.Errorf("Parse() error = nil for %s, want incomplete layer error", name)
		}
	}
}
...
func TestParseConfigContentAllowsSettingsForDisabledFunction(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("Parse() error = %v, want pre-staged settings accepted", err)
	}
...
func TestDefaultConfigRegistersGeneralizedFunctionsDisabled(t *testing.T) {
	...
	for _, name := range []string{
		...
		if enabled {
			t.Errorf("Default()[%q] = true, want opt-in default false", name)
		}
	}
	...
	if err != nil {
		t.Fatalf("Parse() error = %v, want temporal-purity accepted", err)
	}
...
func TestConfigTieBreakerTokens(t *testing.T) {
	...
	custom, err := Parse([]byte(settingsConfig))
	if err != nil {
		t.Fatalf("Parse() error = %v", err)
	}
...
func TestConfigTemporalPurityCSharpPolicy(t *testing.T) {
	...
	if err == nil {
		t.Fatal("Parse() error = nil, want unsupported csharp-policy error")
	}
}
