...
**Passing proposal (planned history terminology)**:
...
**Validation origin (planned history terminology)**:
...
- Planned **Validation history** belongs to the repository and consumes events produced by the **client**; each recorded validation MUST retain its submitted version and verdict for comparison, including speculative proposals.
...
- The planned **client history** interface MUST support listing records, inspecting a record, and diffing two explicitly selected record IDs, with JSON output available; comparison MUST NOT infer that one attempt caused another to pass.
...
*Authored By Peter O'Connor with Assistance from Codex (gpt-6) · 2026-09-06 · Planned repository validation history vocabulary*
