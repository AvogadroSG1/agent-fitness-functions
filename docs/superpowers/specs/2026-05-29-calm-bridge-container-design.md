...
**Local `.calm/config.json` remains** as a developer sandbox. The local `calm-bridge` binary (without `CALM_BRIDGE_ADDR` set) still reads it for fast local iteration. The container is the enforcement point; local config is a DX affordance.

### Runtime-mounted config, hot-reloaded

The governance team does not restart the container to activate a threshold change. An `fsnotify` watcher monitors `CALM_CONFIGS_DIR`. Any file change triggers an in-memory reload within ~1 second. The next `/check` request uses the new config.
...
| `CALM_CONFIGS_DIR` not set | Defaults to `/app/configs` (container-safe path) |
...
graph TD
    ...
    subgraph "CI / Pre-commit Hook"
        HK["pre-commit hook\nCALM_BRIDGE_ADDR=https://container:7890\nCALM_ALLOW_REMOTE_BRIDGE=1\nmTLS client cert"]
    end
...
- Per-client rate limit: 100 requests/minute (configurable via `CALM_RATE_LIMIT`)
- Per-repo concurrency cap: 10 concurrent analyses
- Analyzer timeout: 30s per file (configurable via `CALM_ANALYZER_TIMEOUT`)
...
```bash
export CALM_BRIDGE_ADDR=https://calm-governance.internal:7890
...
6. **`hooks/pre-commit.sh`** — pass `--repo` as logical name (basename or `CALM_REPO_NAME`); remove filesystem path validation for remote addr case; support mTLS client cert env vars.
