...
- **Related:** ADR 0001 (calm-bridge → agent-fitness-functions rename); calm-poc-qo7 (sibling bug, `bin/agent-fitness-functions-test` mTLS — already fixed)

## Context

Two distinct defects in `agent-fitness-functions client install-hooks` were found during
cross-language server testing. Both are fallout from the `calm-bridge → agent-fitness-functions`
...
inconsistent with the `agent-fitness-functions` binary, commands, env vars, and helper
scripts.
...
(`addr=${AGENT_FITNESS_FUNCTIONS_ADDR:-}`) and pass `--addr` only when set. With no
...
`AGENT_FITNESS_FUNCTIONS_ADDR` plus the `CLIENT_*` cert vars.

The sibling helper `bin/agent-fitness-functions-test` already solved this exact problem
...
1. **Rename generated artifacts, recognize legacy on upgrade.** New installs write
   `agent-fitness-functions-pre-commit`, `agent-fitness-functions-pre-push`,
   `agent-fitness-functions-git-guard` and stamp
   `# agent-fitness-functions <hook> hook (sidecar)` markers. Existing-hook **detection**
...
2. **Mirror the test-helper scheme/mTLS convention in generated hooks.** Change
   ...
   `bin/agent-fitness-functions-test` does:
   - `addr` default `https://127.0.0.1:7890` (still overridable via
     `AGENT_FITNESS_FUNCTIONS_ADDR`).
   - cert-dir discovery: `AGENT_FITNESS_FUNCTIONS_DEV_CERT_DIR`, else `<repo>/certs`.
   - per-credential env override precedence:
     `AGENT_FITNESS_FUNCTIONS_CLIENT_CERT/KEY/CA` win; else
...
- Introduce a single naming constant/prefix `agent-fitness-functions` for hook artifact
  names and markers; replace the three `calm-*` literals and the `# CALM ...` /
  `CALM <hook> hook` / `calm-git-guard` literals.
- `installGitHook`: write sidecar `agent-fitness-functions-<hook>`; new marker
  `# agent-fitness-functions <hook> hook (sidecar)`. Detection branch matches the new
  marker **OR** the legacy `CALM <hook> hook` / `# CALM <hook> hook (sidecar)` markers.
- `installGitGuard` / `upsertGitGuard`: write `agent-fitness-functions-git-guard`; when
  scanning existing `settings.json` PreToolUse entries, treat a command containing either
  `agent-fitness-functions-git-guard` **or** legacy `calm-git-guard` as the managed entry
...
- `addr=${AGENT_FITNESS_FUNCTIONS_ADDR:-https://127.0.0.1:7890}` (was empty).
...
- Comment markers in the script headers updated to `# agent-fitness-functions ...`.

### Data flow (unchanged shape, corrected defaults)

```
install-hooks → writes agent-fitness-functions-{pre-commit,pre-push,git-guard}
                + .claude/settings.json PreToolUse → agent-fitness-functions-git-guard
...
- Genuinely foreign (non-managed) hook: existing overwrite/append guard
  (`AGENT_FITNESS_FUNCTIONS_HOOK_OVERWRITE` / `_APPEND`) is preserved unchanged.

## Testing (BDD, red-green-refactor)

1. **Naming (new install):** `install-hooks` on a clean repo writes
   `agent-fitness-functions-pre-commit/-pre-push/-git-guard` and a
   `# agent-fitness-functions pre-commit hook (sidecar)` marker; asserts no `calm-` hook
...
4. **Hook cert discovery:** with `<repo>/certs/{client.crt,client.key,ca.crt}` present,
   ...
   `TestAgentFitnessFunctionsTestPassesMTLS` / `…EnvOverridesCerts` shape.)
