...
| Threshold | Set after baseline. Starting point: ≤ 15 public methods |
| Operator | lte |
...
Enforces a minimum average implementation LOC per public method. Modules with very little logic per public method are likely pass-through facades with no real functionality.

| Property | Value |
|---|---|
| Unit | Per-module |
| Threshold | Set after baseline. Starting point: ≥ 5 LOC per public method |
| Operator | gte |
...
Generated by the Bridge per analysis run. Represents the proposed state of one CALM node (module).

```json
{
  "calm-version": "2024-04",
  "repo": "graft",
  "calm-node": "internal/parser",
  "language": "go",
