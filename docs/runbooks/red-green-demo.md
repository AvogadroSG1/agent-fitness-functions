...
> **Local binary alternative (sandbox only):** Build the binary locally with `go build -o .tmp/calm-bridge ./cmd/stack-fitness-functions` and start it with `.tmp/calm-bridge server start --addr 127.0.0.1:7890`. Use `CALM_BRIDGE_BIN=.tmp/calm-bridge CALM_BRIDGE_ADDR=http://127.0.0.1:7890` for commits. This path is only valid for developer sandbox iteration; it MUST NOT be used as the production path.
